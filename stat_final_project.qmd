...
Furthermore, we must check the independence assumption. While all samples were taken within the town of Framingham, MA, every individual did not live in the same community; instead, letters were sent out randomly inviting famlies to participate in the study. Since the sampling was relatively random, for our purposes we may say that the independence assumption is satisfied. 

Now, we search for influential values by Cook's distance.
```{r mode-4, message = F, warning = F, echo = F}
plot(m2, which = 4, id.n = 3)
```
We note that there are 3 high Cook's Distance values which we want to investigate: 
```{r mode-5, message = F, warning = F, echo = F, results = F}
m2_aug_s <- augment(m2) %>% 
    mutate(index = 1:n()) 
